...
HomeScreen::HomeScreen()
{
    ...
    social_button.SetSize(Vector2f(menu_background.GetSize().x - 20, button_height - 5));
    social_button.SetFillColor(BUTTON_BACKGROUND_COLOR_V2);
    social_button.SetPosition(play_button_shade.GetEndPosition() +
                              Vector2f(-play_button_shade.GetSize().x, button_height_space));
    social_button.SetCornerSize(10);

    social_button_shade.SetSize(Vector2f(menu_background.GetSize().x - 20, button_height));
    social_button_shade.SetFillColor(DarkenColor(BUTTON_BACKGROUND_COLOR_V2, 0.7));
    social_button_shade.SetPosition(social_button.GetPosition());
    social_button_shade.SetCornerSize(10);
    ...
    options_button.SetPosition(social_button_shade.GetEndPosition() +
                               Vector2f(-social_button_shade.GetSize().x, button_height_space));
    ...
    social_text.SetString("Social");
    social_text.SetCharacterSize(40);
    social_text.Center(social_button.GetPosition(), social_button.GetSize());
...
void HomeScreen::Animate(RenderWindow &window)
{
    ...
    if (play_button.Contains(mouse_pos))
        ...
        play_button.SetFillColor(BUTTON_BACKGROUND_COLOR);
    if (social_button.Contains(mouse_pos))
        social_button.SetFillColor(DarkenColor(BUTTON_BACKGROUND_COLOR_V2, 0.9));
    else
        social_button.SetFillColor(BUTTON_BACKGROUND_COLOR_V2);
    ...
    if (Mouse::isButtonPressed(Mouse::Button::Left))
    {
        if (play_button.Contains(mouse_pos))
            ...
            play_button.SetFillColor(BUTTON_BACKGROUND_COLOR);
        if (social_button.Contains(mouse_pos))
            social_button.SetFillColor(DarkenColor(BUTTON_BACKGROUND_COLOR_V2, 0.8));
        else
            social_button.SetFillColor(BUTTON_BACKGROUND_COLOR_V2);
        if (options_button.Contains(mouse_pos))
...
unsigned int HomeScreen::LogicOnClick(Vector2f mouse_pos)
{
    if (Mouse::isButtonPressed(Mouse::Button::Left))
    {
        ...
        else
        {
            ...
            else if (social_button.Contains(mouse_pos))
            {
                return ScreenCommand::SocialS;
            }
            else if (exit_button.Contains(mouse_pos))
            {
                return ScreenCommand::ExitS;
            }
        }
    }
...
void HomeScreen::Draw(RenderWindow &window)
{
    ...
    social_button_shade.Draw(window);
    social_button.Draw(window);
